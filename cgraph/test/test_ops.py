...
def test_pow():
    ...
    d = f.sdiff()
    checkf(d[x], {x:2, y:3}, value=12, ngrad={x: 12, y:4+math.log(4096)})                                      # tests ddf/dxdx and ddf/dxdy
    checkf(d[y], {x:2, y:3}, value=math.log(256), ngrad={x:4+math.log(4096), y:8 * math.log(2) * math.log(2)}) # tests ddf/dydx and ddf/dydy

def test_exp():

    x = sym.Symbol('x')
    f = cgraph.ops.exponential.exp(x) / x
